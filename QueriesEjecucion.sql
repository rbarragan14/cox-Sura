...
FROM SCRIPT (
ON (select  
 1 c1,Cross_Validation,Asegurado_id ,  Ind_Ramo_Canal_Vigente, Meses_Antiguedad_Ramo_Canal, Canal_Agrup_Id 
...
,Ind_Forma_Pago_Mensual, Ind_Forma_Pago_Anual, Ind_Tipo_Pago_Caja_Sura, Ind_Tipo_Pago_Financiacion_Sura 
FROM Rtdsura.Training_Modelo_VPC_Autos2
)
partition by c1
...
RETURNS ('Aseg integer, T1 decimal (18,4) , T2 decimal (18,4)')) a
INNER JOIN 
Rtdsura.Training_Modelo_VPC_Autos2 b
on b.Asegurado_id = a.Aseg
;
...
SELECT DISTINCT *
    FROM SCRIPT (
    SCRIPT_COMMAND('tail -n 19 /var/opt/teradata/tdtemp/uiflib/scriptlog')
